...
select D.department_id , D.department_name , E.last_name from employees as E join departments as D on E.department_id = D.department_id;

-- 2. Create a unique list of all jobs that are in department 4. Include the location of the department in the output. 
select distinct E.job_id , L.city as Location from employees E join departments D on E.department_id = D.department_id join locations L on D.location_id = L.location_id where E.department_id =4;  
...
select E.last_name , D.department_name, L.location_id , L.city , commission_pct  from employees As E join departments As D on  E.department_id = D.department_id join locations L on D.location_id = L.location_id where commission_pct is not null;
-- 4. Display the employee last name and department name of all employees who have an 'a' in their last name 
select E.last_name , D.department_name from employees As E join departments As d on E.department_id = D.department_id where E.last_name like "%a%";
-- 5. Write a query to display the last name,job,department number and department name for all employees who work in ATLANTA. 
select E.last_name , E.job_id , D.department_id , D.department_name  from employees As E join departments As d on E.department_id = D.department_id join locations as L on D.location_id = L.location_id where L.city = 'ATLANTA';
...
select E.first_name , E.last_name , E.job_id , D.department_name , E.salary , case when E.salary >= 50000 then "A" when E.salary >= 30000 then "B" else "C" end as Grade from employees  E join departments  D on  E.department_id = D.department_id ; 
