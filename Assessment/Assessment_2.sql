...
select category , count(*) from sales group by category having count(*)>10;
...
-- 5-	How many sales does each product have?
 select count(*), product_name from sales group by product_name;
...
select category , max(sale_amount) , product_name from sales group by category,product_name; 
-- 13-	How many sales occurred for each month?
select month(sale_date),count(*) from sales group by month(sale_date);
-- 14-	In which months are there more than 5 sales?
select month(sale_date),count(*) from sales group by month(sale_date) having count(*)>5;
-- 15-	What is the average sale amount for each product category in months with more than 3 sales?
select avg(sale_amount), product_name , month(sale_date) from sales group by product_name , month(sale_date) having count(*)>3;
-- 16-	Which products have more than 1 sale in every recorded month?
select product_name  from sales group by product_name having count(distinct month(sale_date)) = ( select count(distinct month(sale_date)) from sales) and count(*)>1;
-- 17-	What is the month with the lowest total sales amount?
select month(sale_date) , sum(sale_amount) from sales group by month(sale_date) order by sum(sale_amount) asc limit 1;
...
select category , sum(sale_amount) , month(sale_date) from sales group by category , month(sale_date) order by sum(sale_amount) desc;
-- 20-	Which products have sale amounts above the average sale amount across all products?
select product_name , avg(sale_amount) from sales group by product_name having avg(sale_amount)>(select avg(sale_amount) from sales);
...
select product_name from sales group by product_name having count(distinct month(sale_date));
-- 24-	Which categories have an increasing total sales amount across months?
select category , sum(sale_amount) , month(sale_date) from sales group by category;
-- 25-	In which months is there a 20% increase in total sales amount compared to the previous month?
select month(sale_date) , (max(sale_amount)-min(sale_amount))/min(sale_amount) as sale_increase from sales group by month(sale_date) having sale_increase>0.20;
...
SELECT category FROM sales GROUP BY category HAVING COUNT(DISTINCT month(sale_date)) = (SELECT COUNT(DISTINCT year(sale_date)) from sales);
...
select product_name , sale_amount , count(distinct category) from sales group by product_name , sale_amount having count(distinct category)>1;
