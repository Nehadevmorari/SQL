...
select count(O.orderid) , C.firstname , C.lastname from customers as C left join  orders as  O  on C.customerid = O.customerid group by C.firstname , C.lastname;
...
select O.orderid , O.quantity , P.productname  from orders as O  join  products as  P  on O.productid = P.productid;

-- 8. List the customers who have not placed any orders.
select C.firstname , O.customerid from orders as O  join  customers as  C  on O.customerid = C.customerid where O.orderid is null;
...
select O.orderid , C.country from orders as O  join  customers as  C  on O.customerid = C.customerid where country = 'USA';
...
select C.firstname , C.state , O.orderid from orders as O  join  customers as  C  on O.customerid = C.customerid where state = 'California';

-- 14. Show the customers along with the number of orders they've placed.
select C.customerid , C.firstname , count(O.orderid) from orders as O  join  customers as  C  on O.customerid = C.customerid group by C.customerid , C.firstname;

-- 15. Display products released after a specific date (e.g., '2022-01-01').
select productid , productname , releasedate from products where releasedate > '2022-01-01';

-- 16. Retrieve the orders with a specific payment status (e.g., 'Paid').
select orderid , paymentstatus from orders where paymentstatus = 'Paid';
...
select productid , productname , weight from products;

-- 19. Retrieve the orders placed by customers from a specific state (e.g., 'California').
-- 20. Display products with low stock quantity (e.g., less than 10).
select productid , productname , stockquantity from products where stockquantity<10;

-- 21. Retrieve the orders placed on a specific date (e.g., '2023-01-15').
select orderid , orderdate from orders where orderdate = '2023-01-15';
...
SELECT C.FirstName, C.LastName, P.Category
FROM Customers As C
...
select orderid , paymentstatus from orders where paymentstatus = 'Pending';
...
select O.orderid , C.firstname , C.lastname , c.zipcode from orders As O join customers As C on O.customerid = C.customerid where c.zipcode = '12345';
-- 28. List the products with prices between $50 and $100.
select productid , productname ,price from products where price between 50 and 100;
...
select productid , productname , stockquantity from products where stockquantity>20;